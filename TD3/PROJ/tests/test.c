...
#define EXE_PATH "../main/TDfinal.exe"  // Caminho relativo


// Executa o programa com input redirecionado e captura o output
int run_TDfinal(const char *input_file, const char *output_file) {
    char cmd[512];
...
void test_output_won(void) { 
    int exit_status = run_TDfinal("tests/WON.txt", "tmp_won.txt");
...
void test_output_waiting(void) { 
    int exit_status = run_TDfinal("tests/WAITING.txt", "tmp_waiting.txt");
    CU_ASSERT_EQUAL(exit_status, 0);
...
void test_output_failed(void) { 
    int exit_status = run_TDfinal("tests/FAILED.txt", "tmp_failed.txt");
    CU_ASSERT_EQUAL(exit_status, 0);
...
void test_maxchoices_error(void) { 
    int exit_status = run_TDfinal("tests/MAXCHOICES-ERROR.txt", "tmp_maxchoices.txt");
...
void test_maxtxt_error(void) { 
    int exit_status = run_TDfinal("tests/MAXTXT-ERROR.txt", "tmp_maxtxt.txt");
    CU_ASSERT_EQUAL(exit_status, 1); // Espera status de erro
